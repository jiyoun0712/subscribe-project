...
import { UpdatePost } from '@/store/apps/post/PostSlice';
...
const UpdatePostDialog: React.FC<DetailDialogProps> = ({ open, onClose }) => {

  const postDetail: PostType = useSelector(
        (state) => state.postReducer.post.find((post) => post.p_no === state.postReducer.postContent)
    );

  const dispatch = useDispatch();
  const quillRef = useRef<any>(null);
  
  const [scolor, setScolor] = useState<string>(postDetail.color);
 // const id = useSelector((state) => state.prayersReducer.prayers.length + 1);
  ...
  const [inputValue, setInputValue] = useState<string>(postDetail.contents);
      ...
      >
        ...
        <Scrollbar sx={{ height: "calc(100vh - 5px)" }}>
          <Box
            ...
            alignItems="center"
          >
            <Typography variant="h4">기도 수정하기</Typography>
          ...
          <Box p={3}>
            ...
            <Stack direction={"row"} gap={2} my={2}  sx={{justifyContent: 'flex-end'}}>   
                ...
                onClick={(e) => {
                  e.preventDefault();
                  dispatch(UpdatePost(postDetail.p_no, inputValue, scolor));
                  onClose(); // 다이얼로그 닫기
